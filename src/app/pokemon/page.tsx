...
export default function PokemonPage() {
  ...
  useEffect(() => {
    ...
    const fetchList = async () => {
      // Para permitir navegación sin filtros, necesitamos cargar más Pokémon
      const total = search.length > 0 || generation || typeFilter || typeFilter2 ? 1010 : Math.max(1010, limit * page);
      try {
  ...
  <div className="flex flex-col md:flex-row gap-3 mb-6 items-center flex-wrap">
      ...
      <div className="flex justify-center gap-4 mt-8">
        ...
        <button
          ...
          onClick={() => {
            const totalPages = search.length > 0 || generation || typeFilter || typeFilter2 
              ? Math.ceil(filteredPokemons.length / limit)
              : Math.ceil(1010 / limit); // Total real de Pokémon
            setPage((p) => p < totalPages ? p + 1 : p);
          }}
          disabled={(() => {
            const totalPages = search.length > 0 || generation || typeFilter || typeFilter2 
              ? Math.ceil(filteredPokemons.length / limit)
              : Math.ceil(1010 / limit);
